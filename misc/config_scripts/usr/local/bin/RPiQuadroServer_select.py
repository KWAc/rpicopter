...
def handle_socket(udp_socket):
  if udp_socket is udp_server: 
    client, addr = server.accept() 
    udp_clients.add(client) 
  else: 
    udp_msg = udp_socket.recv(512) 
    if udp_msg: 
      try: 
...
def trnm_thr():                                                         # trnm_thr() sends commands to Arduino and is a multiplexed socket server
  ...
  while pySerial is not None:
    ...
    udp_rd_socket, udp_wr_socket, oob = select.select(udp_clients, [], [])
    for udp_socket in udp_rd_socket: 
      handle_socket(udp_socket)
